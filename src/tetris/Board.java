...
public class Board extends JPanel implements KeyListener, MouseListener, MouseMotionListener {
    ...
    public  int boardHeigh, boardWidth ;
    ...
    public Board(int H, int W) {
        ...
        board  = new Color[boardHeigh][boardWidth];
        ...
        stopBounds = new Rectangle(500, 550, pause.getWidth(), pause.getHeight() + pause.getHeight() / 2);
        refreshBounds = new Rectangle(500, 550 - refresh.getHeight() - 20, refresh.getWidth(),
    ...
    public void paintComponent(Graphics g) {
        ...
        for (int row = 0; row < nextShape.getCoords().length; row++) {
            for (int col = 0; col < nextShape.getCoords()[0].length; col++) {
                if (nextShape.getCoords()[row][col] != 0) {
                    g.fillRect(col * 30 + (boardWidth+1)*30, row * 30 + 50, Board.blockSize, Board.blockSize);
                }
            }
        }
        ...
        if (gamePaused) {
            ...
            g.drawString(gamePausedString, 15, WindowGame.HEIGHT / 2);
            }else{
                     String gamePausedString = "GAME PAUSED";
            g.setColor(Color.orange);
            g.setFont(new Font("Verdana", Font.BOLD, 34));
            g.drawString(gamePausedString, 85, WindowGame.HEIGHT / 2);
            }
        ...
        if (gameOver) {
            ...
            g.setFont(new Font("Verdana", Font.BOLD, 34));
            g.drawString(gameOverString, 20, WindowGame.HEIGHT / 2);
            }else{
                         String gameOverString = "GAME OVER";
            g.setColor(Color.GREEN);
            g.setFont(new Font("Verdana", Font.BOLD, 34));
            g.drawString(gameOverString, 85, WindowGame.HEIGHT / 2);
                    }
        ...
        g.drawString(score + "", WindowGame.WIDTH - 110, WindowGame.HEIGHT / 2 + 30);
        ...
        for (int j = 0; j <= boardWidth; j++) {
            g.drawLine(j * blockSize, 0, j * blockSize, boardHeigh * 30);
        }
    }

    public void setNextShape() {//set cho khoi ngau nhien tiep theo 
        int index = random.nextInt(shapes.length);
        int colorIndex = random.nextInt(colors.length);
        nextShape = new Shape(shapes[index].getCoords(), this, colors[colorIndex]);
    }

    public void setCurrentShape() {//set cho khoi moi xuat hien 
